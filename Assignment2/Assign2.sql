...
JOIN Orders AS o ON c.CustomerID = o.CustomerID
JOIN [Order Details] AS od ON o.OrderID = od.OrderID
WHERE o.OrderDate >= DATEADD(year, -27, GETDATE()) AND c.PostalCode is NOT NULL
GROUP BY c.PostalCode
ORDER BY OrdersCount DESC;
...
FROM Suppliers AS s
JOIN Shippers AS sc ON s.SupplierID = sc.ShipperID;
...
SELECT e.FirstName + ' ' + e.LastName AS Manager, COUNT(emp.EmployeeID) AS EmployeesCount
FROM Employees AS e
